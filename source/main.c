...
void TickFct_screen(){
		switch (state){
			case s_init:
				state = off;
				cnt = score = reaction = 0;
				lcd_setXY(0x40, 0x80);
				N5110_Data("Press button  to start");
				break;
			case off:
				if ((~PINA & 0x08) == 0x08){
					//PORTD = 0x02;
		...
		switch (state){
			...
			case gameOver:
				...
				sprintf(buffer, "%d", reaction);
				N5110_Data("       Avg Reaction  Time:");
				N5110_Data(buffer);
                                N5110_Data("      Press button  to play again");
				break;
			case gameOverWait:
				//PORTD = 0x08;
				break;
